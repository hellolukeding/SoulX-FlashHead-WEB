"""
CosyVoice-300M-SFT TTS 服务 - 使用 vLLM 加速
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    global cosyvoice_model, model_loaded, available_speakers

    logger.info("🎵 初始化 CosyVoice-300M-SFT 模型 (vLLM 加速)...")
    ...
    try:
        ...
        # 使用 SFT 模型
        model_path = 'iic/CosyVoice-300M-SFT'
        ...
        # 获取预设音色列表
        available_speakers = cosyvoice_model.list_available_spks()

        model_loaded = True
        logger.info(f"✅ CosyVoice SFT 模型加载成功")
        logger.info(f"   采样率: {cosyvoice_model.sample_rate}Hz")
        logger.info(f"   预设音色数: {len(available_speakers)}")
        logger.info(f"   vLLM 加速: 已启用")

    except Exception as e:
        logger.error(f"❌ 模型加载失败: {e}")
        logger.error("服务将以无模型模式启动")
        import traceback
        traceback.print_exc()
...
app = FastAPI(
    title="CosyVoice SFT TTS 服务 (vLLM 加速)",
    description="支持预设音色的 TTS 服务，使用 vLLM 加速",
    version="2.0.0",
    lifespan=lifespan,
)
...
async def text_to_speech(request: TTSRequest):
    ...
    if request.speaker not in available_speakers:
        raise HTTPException(
            status_code=400,
            detail=f"无效的音色: {request.speaker}。可用音色: {available_speakers[:10]}..."
        )

    start_time = time.time()

    try:
        logger.info(f"[TTS] 收到请求: {request.text[:50]}... (音色: {request.speaker})")
...
async def text_to_speech_streaming(request: StreamingTTSRequest):
    ...
    if request.speaker not in available_speakers:
        raise HTTPException(
            status_code=400,
            detail=f"无效的音色: {request.speaker}"
        )

    logger.info(f"[TTS Stream] 收到请求: {request.text[:50]}... (音色: {request.speaker})")
...
async def root():
    """根路径"""
    return {
        "service": "CosyVoice SFT TTS 服务 (vLLM 加速)",
        "version": "2.0.0",
        "mode": "sft",
        "vllm_enabled": True,
        "endpoints": {
