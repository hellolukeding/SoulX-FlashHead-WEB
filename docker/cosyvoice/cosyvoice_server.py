...
class TTSRequest(BaseModel):
    ...
    prompt_audio_url: str = ""  # 可选，留空使用默认
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    global cosyvoice_model, model_loaded, default_prompt_wav
    ...
    try:
        ...
        # 加载默认参考音频
        default_prompt_path = '/models/CosyVoice/pretrained_models/CosyVoice-300M/asset/default_prompt.wav'
        if os.path.exists(default_prompt_path):
            default_prompt_wav = default_prompt_path
            logger.info(f"✅ 加载默认参考音频: {default_prompt_path}")
        else:
            logger.warning("⚠️ 未找到默认参考音频，请求需要提供 prompt_audio_url")
...
app = FastAPI(
    ...
    description="支持流式切片输出的 Zero-shot TTS 服务",
    version="2.0.0",
    lifespan=lifespan,
)
...
async def health_check():
    ...
    return {
        ...
        "default_prompt_available": default_prompt_wav is not None,
    }


@app.post("/tts")
async def text_to_speech(request: TTSRequest):
    """标准 TTS 端点 - Zero-shot 模式"""
    if cosyvoice_model is None:
        raise HTTPException(
            status_code=503,
    ...
    try:
        ...
        # 确定使用的参考音频
        ref_wav = request.prompt_audio_url if request.prompt_audio_url else default_prompt_wav
        if not ref_wav:
            raise HTTPException(
                status_code=400,
